...
function setCors(res, origin) {
  ...
  res.setHeader("Access-Control-Allow-Headers", "authorization,content-type,accept-language,x-user-id,x-requested-with");
...
export default async function handler(req, res) {
  ...
  if (!origin || !apiKey) {
    res.status(500).json({ error: { code: "PROXY_NOT_CONFIGURED", message: "Proxy serveur non configure." } });
    return;
  }
  ...
  try {
    ...
    res.status(502).json({ error: { code: "UPSTREAM_UNAVAILABLE", message: "Serveur Tissint indisponible." } });
  }
}
