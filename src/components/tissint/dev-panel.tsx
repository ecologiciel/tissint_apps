...
export function DevPanel() {
  ...
  return (
    <>
      ...
      {devPanelOpen && (
        <div
          className="fixed bottom-16 left-4 z-50 w-72 rounded-2xl bg-stone text-warm shadow-2xl p-4 border border-white/10"
          dir="rtl"
        >
          ...
          <div className="space-y-3 text-xs">
            <div>
              <p className="opacity-70 mb-1">
                API: <span className="font-mono">{TISSINT_API_MODE}</span>
              </p>
              <p className="opacity-70">
                المسح اليومي: {scansToday}/{dailyLimit === 999 ? "∞" : dailyLimit}
              </p>
              <button onClick={resetScans} className="mt-1 text-orange underline">
                إعادة تصفير
              </button>
            </div>
            <div>
