  ...
  return (
    <div className="flex h-full flex-col bg-stone text-warm" dir="rtl">
      <div className="relative px-5 pt-10 pb-6">
        {back && (
          <Link to={back as never} className="absolute top-10 right-5 grid h-9 w-9 place-items-center rounded-full bg-white/10">
...
export function AuthInput({
  label, type = "text", value, onChange, placeholder, icon: Icon, dir = "rtl", maxLength,
}: {
  label: string; type?: string; value: string;
  onChange: (v: string) => void; placeholder?: string;
  icon?: any; dir?: "rtl" | "ltr"; maxLength?: number;
}) {
  return (
...
export function AuthButton({
  children, onClick, variant = "primary", loading, type = "button", disabled,
}: {
  children: ReactNode; onClick?: () => void;
  variant?: "primary" | "ghost" | "social";
  loading?: boolean; type?: "button" | "submit"; disabled?: boolean;
}) {
  const base = "w-full rounded-xl py-3 text-sm font-bold transition active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2";
  const styles = {
