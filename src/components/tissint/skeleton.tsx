...
export function EmptyState({
  ...
  const btnClass = "mt-4 inline-flex items-center justify-center rounded-full bg-gradient-to-r from-orange to-gold text-white text-xs font-bold px-5 py-2.5 shadow-md";
  return (
    <div className="rounded-3xl bg-card border border-border p-8 text-center" dir="rtl">
      ...
      {body && <p className="mt-1 text-xs text-muted-foreground leading-5 max-w-xs mx-auto">{body}</p>}
      {actionLabel && actionTo && (
        <Link to={actionTo as never} className={btnClass}>{actionLabel}</Link>
      )}
...
export function SectionLoader({ children }: { children?: ReactNode }) {
  return (
    <div className="space-y-2 animate-pulse">{children ?? Array.from({ length: 3 }).map((_, i) => <RowSkeleton key={i} />)}</div>
  );
}
