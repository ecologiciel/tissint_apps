...
import type {
  UserRole, ScenarioKey, CollectionItem, Listing, ScanResult,
  Conversation, ChatMessage, AppNotification,
  PaymentMethod, Transaction, Invoice, PlanKey,
} from "./tissint-types";
import {
  MOCK_COLLECTION, MOCK_LISTINGS,
  MOCK_CONVERSATIONS, MOCK_MESSAGES, MOCK_NOTIFICATIONS,
  MOCK_PAYMENT_METHODS, MOCK_TRANSACTIONS, MOCK_INVOICES,
} from "./mock-data";
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const removeFromCollection = useCallback((id: string) => setCollection((c) => c.filter((x) => x.id !== id)), []);
  ...
  const sendMessage = useCallback((threadId: string, text: string) => {
    ...
    const msg: ChatMessage = {
      ...
      threadId, fromMe: true, text, createdAt: now, read: true,
    };
    setMessages((ms) => [...ms, msg]);
    setConversations((cs) => cs.map((c) =>
      c.id === threadId ? { ...c, lastMessage: text, lastAt: now } : c
    ));
    // Mock auto-reply after a short delay
    setTimeout(() => {
      const reply: ChatMessage = {
        id: "m_" + Math.random().toString(36).slice(2, 9),
        threadId, fromMe: false,
        text: "شكراً لتواصلك، سأرد عليك قريباً.",
        createdAt: new Date().toISOString(), read: false,
      };
      setMessages((ms) => [...ms, reply]);
      setConversations((cs) => cs.map((c) =>
        c.id === threadId ? { ...c, lastMessage: reply.text, lastAt: reply.createdAt, unread: c.unread + 1 } : c
      ));
    }, 1400);
  }, []);
  ...
  const startConversationForListing = useCallback((l: Listing): string => {
    const existing = conversations.find((c) => c.listingId === l.id);
    if (existing) return existing.id;
    const id = "th_" + Math.random().toString(36).slice(2, 9);
    const conv: Conversation = {
      id, listingId: l.id, listingTitle: l.title, listingImageSeed: l.imageSeed,
      peerName: l.sellerName, peerVerified: l.sellerVerified,
      lastMessage: "محادثة جديدة", lastAt: new Date().toISOString(), unread: 0,
    };
  ...
  const unreadMessages = useMemo(() => conversations.reduce((s, c) => s + c.unread, 0), [conversations]);
  ...
  const setDefaultPaymentMethod = useCallback((id: string) =>
    setPaymentMethods((pms) => pms.map((p) => ({ ...p, isDefault: p.id === id }))), []);
  const addPaymentMethod = useCallback((pm: Omit<PaymentMethod, "id">) =>
    setPaymentMethods((pms) => [...pms, { ...pm, id: "pm_" + Math.random().toString(36).slice(2, 8) }]), []);
  const removePaymentMethod = useCallback((id: string) =>
    setPaymentMethods((pms) => pms.filter((p) => p.id !== id)), []);

  const topUpWallet = useCallback((amountDh: number, methodId: string): Transaction => {
    const tx: Transaction = {
      id: "tx_" + Math.random().toString(36).slice(2, 8),
      kind: "topup",
      label: "شحن المحفظة (" + (paymentMethods.find((p) => p.id === methodId)?.label ?? "—") + ")",
      amountDh, status: "completed", createdAt: new Date().toISOString(),
    };
    setTransactions((ts) => [tx, ...ts]);
    return tx;
  }, [paymentMethods]);

  const subscribePremium = useCallback((plan: PlanKey, methodId: string) => {
    ...
    const tx: Transaction = {
      id: "tx_" + Math.random().toString(36).slice(2, 8),
      kind: "premium",
      label: invoice.label + " (" + (paymentMethods.find((p) => p.id === methodId)?.label ?? "—") + ")",
      amountDh: -priceDh,
      status: "completed",
      createdAt: now.toISOString(),
  ...
  return (
    <Ctx.Provider value={{
      role, onboarded, userName, scenario, scansToday, dailyLimit,
      collection, listings, lastScan, devPanelOpen,
      conversations, messages, notifications, unreadMessages, unreadNotifications,
      paymentMethods, transactions, invoices, walletBalanceDh, premiumPlan, premiumRenewsAt,
      favoriteIds, toggleFavorite,
      setRole, setOnboarded, setUserName, setScenario,
      incrementScans, resetScans, setLastScan,
      addToCollection, removeFromCollection, addListing, updateListingStatus,
      toggleDevPanel,
      sendMessage, markThreadRead, startConversationForListing,
      markNotificationRead, markAllNotificationsRead,
      setDefaultPaymentMethod, addPaymentMethod, removePaymentMethod,
      topUpWallet, subscribePremium, cancelPremium,
    }}>
      {children}
    </Ctx.Provider>
  );
