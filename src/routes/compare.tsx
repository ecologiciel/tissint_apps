...
function ComparePage() {
  ...
  const items = selectedIds.map((id) => collection.find((c) => c.id === id)).filter(Boolean) as typeof collection;
  ...
  const rows: { label: string; get: (c: any) => string; numeric?: boolean; numVal?: (c: any) => number }[] = [
    ...
    { label: "الوزن", get: (c) => (c.weightG ? `${c.weightG} غ` : "—"), numeric: true, numVal: (c) => c.weightG || 0 },
  ...
  return (
    <div className="flex h-full flex-col bg-background" dir="rtl">
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-3">
        {/* Headers */}
        <div className="grid gap-2" style={{ gridTemplateColumns: `100px repeat(${Math.max(items.length, 1)}, 1fr) ${items.length < 3 ? "60px" : ""}` }}>
          <div />
          {items.map((c) => (
            <div key={c.id} className="rounded-xl bg-card border p-2 text-center relative">
              <button onClick={() => remove(c.id)} className="absolute top-1 left-1 grid h-5 w-5 place-items-center rounded-full bg-destructive/15 text-destructive">
                <X className="h-3 w-3" />
              </button>
        ...
        {items.length > 0 ? (
          <div className="rounded-2xl bg-card border overflow-hidden">
            {rows.map((row, ri) => {
              ...
              const bestIdx = row.numeric && row.numVal
                ? items.reduce((bi, c, i) => row.numVal!(c) > row.numVal!(items[bi]) ? i : bi, 0)
                : -1;
              return (
                <div
                  key={row.label}
                ...
                >
                  ...
                  {values.map((v, i) => (
                    <div key={i} className={`text-center ${bestIdx === i ? "font-black text-success" : ""}`}>
      ...
      {/* Picker sheet */}
      {picker && (
        <div className="absolute inset-0 z-50 bg-black/60 flex items-end" onClick={() => setPicker(false)}>
          <div className="w-full bg-card rounded-t-3xl p-4 max-h-[70%] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {collection.filter((c) => !selectedIds.includes(c.id)).map((c) => (
                <button key={c.id} onClick={() => add(c.id)} className="rounded-xl border bg-card p-2 text-right">
...
function verdictLabel(v: string) {
  return ({ likely: "احتمال قوي", possible: "متوسط", unlikely: "ضعيف", rejected: "مرفوض" } as any)[v] || v;
}
