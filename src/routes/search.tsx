...
import {
  ChevronLeft, Search as SearchIcon, X, Store, Library, MessageCircle, ScanLine,
} from "lucide-react";
...
function SearchPage() {
  ...
  const mResults = useMemo(
    () => listings.filter((l) => l.status === "approved" && (match(l.title) || match(l.classification) || match(l.region))),
    [listings, q]
  );
  ...
  const tResults = useMemo(
    () => conversations.filter((c) => match(c.listingTitle ?? "") || match(c.peerName) || match(c.lastMessage)),
    [conversations, q]
  );
  ...
  return (
    <div className="flex h-full flex-col bg-warm" dir="rtl">
      <header className="bg-navy text-warm px-5 pt-12 pb-4 rounded-b-3xl">
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1">
            ...
            {q && (
              <button onClick={() => setQ("")} className="absolute left-2 top-1/2 -translate-y-1/2 grid h-6 w-6 place-items-center rounded-full bg-white/10">
                <X className="h-3 w-3" />
              </button>
            )}
          </div>
      ...
      <main className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            {recent.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={clearRecent} className="text-[10px] text-orange font-bold">مسح</button>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {recent.map((r) => (
                    <button key={r} onClick={() => setQ(r)}
                      className="rounded-full bg-muted px-3 py-1.5 text-xs font-semibold">{r}</button>
                  ))}
                </div>
              </section>
            )}
        ...
        {q !== "" && total === 0 && (
          <EmptyState icon={SearchIcon} title="لا توجد نتائج" body={`لم نجد شيئاً لـ "${q}". جرّب كلمات أخرى.`} />
        )}
              ...
              {mResults.slice(0, 6).map((l) => (
                <Link key={l.id} to="/market/$listingId" params={{ listingId: l.id }}
                  ...
                  <div className="p-2">
                    ...
                    <p className="text-[10px] text-orange font-black">{l.priceDh.toLocaleString()} د.م</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <Section title="الرسائل" count={tResults.length}>
            <div className="space-y-2">
              {tResults.slice(0, 5).map((c) => (
                <Link key={c.id} to="/messages/$threadId" params={{ threadId: c.id }}
                  className="flex items-center gap-3 rounded-xl bg-card border p-2">
                  <MeteoriteThumb seed={c.listingImageSeed ?? "x"} className="h-10 w-10 rounded-lg shrink-0" />
                  <div className="flex-1 min-w-0">
