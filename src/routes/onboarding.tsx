...
const FEATURES = [
  {
    icon: ScanLine,
    title: "امسح حجرك",
    body: "استعمل الكاميرا. الذكاء الاصطناعي يحلل القشرة، الكثافة والمغناطيسية.",
  },
  {
    icon: Sparkles,
    title: "نتيجة فورية",
    body: "تقييم من 0 إلى 100 مع تصنيف مبدئي ونصائح للتأكيد.",
  },
      ...
      <div className="px-6 pb-8">
        <div className="mb-4 flex justify-center gap-1.5">
          {Array.from({ length: totalSteps }).map((_, idx) => (
            <span
              key={idx}
              className={`h-1.5 rounded-full transition-all ${idx === step ? "w-8 bg-orange" : idx < step ? "w-1.5 bg-orange/60" : "w-1.5 bg-white/20"}`}
            />
          ))}
        </div>
