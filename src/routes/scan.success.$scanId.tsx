...
import { Check, ShieldCheck, ShieldAlert, BookmarkPlus, Store, ChevronRight, Camera } from "lucide-react";
  ...
  return (
    <div className="flex h-full flex-col bg-stone text-warm relative overflow-hidden" dir="rtl">
      <div className="pointer-events-none absolute inset-0 z-0">
        {isReady && Array.from({ length: 24 }).map((_, i) => (
      ...
      <main className="flex-1 overflow-y-auto flex flex-col items-center justify-center px-6 text-center relative z-10 py-6">
        <div className={`grid h-24 w-24 place-items-center rounded-full ${accent} shadow-2xl animate-scale-in`}>
        ...
        <div className="mt-6 w-full max-w-sm rounded-2xl bg-white/5 p-4 border border-white/10 space-y-3">
          ...
          <div className="text-right space-y-1.5 text-sm">
            <div className="flex justify-between"><span className="text-warm/60">التصنيف</span><span className="font-bold">{r.classification}</span></div>
            <div className="flex justify-between"><span className="text-warm/60">النتيجة</span><span className={`font-black ${accentText}`}>{r.score}%</span></div>
            <div className="flex justify-between"><span className="text-warm/60">رقم المسح</span><span className="font-mono text-xs">{r.scanId}</span></div>
          </div>
        </div>
      </main>
