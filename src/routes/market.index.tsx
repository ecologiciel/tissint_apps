...
import { BadgeCheck, MapPin, Search, SlidersHorizontal, X, ArrowUpDown, Check } from "lucide-react";
...
function MarketPage() {
  ...
  const filtered = useMemo(() => {
    let out = approved.filter((l) => {
      ...
      if (q && !l.title.includes(q) && !l.classification.includes(q) && !l.region.includes(q))
        return false;
      return true;
    });
    switch (sort) {
      case "price_asc":
        out = [...out].sort((a, b) => a.priceDh - b.priceDh);
        break;
      case "price_desc":
        out = [...out].sort((a, b) => b.priceDh - a.priceDh);
        break;
      case "score":
        out = [...out].sort((a, b) => b.score - a.score);
        break;
      case "weight":
        out = [...out].sort((a, b) => b.weightG - a.weightG);
        break;
      default:
  ...
  const activeFilters =
    ...
    (mode !== "all" ? 1 : 0) +
    (verifiedOnly ? 1 : 0) +
    (priceMin > 0 || priceMax < 100000 ? 1 : 0) +
    (weightMin > 0 ? 1 : 0);
      ...
      <header className="bg-navy text-warm px-5 pt-12 pb-5 rounded-b-3xl">
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2">
            <Link
              to="/price-history"
              className="rounded-full bg-white/10 text-warm text-xs font-bold px-3 py-1.5"
            >
              📈 الأسعار
            </Link>
            <Link
              to="/market/my-listings"
              className="rounded-full bg-orange text-white text-xs font-bold px-3 py-1.5"
            >
              إعلاناتي
            </Link>
          </div>
        ...
        <div className="mt-4 flex gap-2">
          ...
          >
            ...
            {activeFilters > 0 && (
              <span className="absolute -top-1 -right-1 grid h-4 min-w-4 px-1 place-items-center rounded-full bg-gold text-[9px] font-black text-stone">
                {activeFilters}
              </span>
            )}
          </button>
      ...
      {openFilters && (
        ...
        >
          <div
            ...
            className="bg-card rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto"
            dir="rtl"
          >
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    placeholder="من"
                    className="rounded-xl bg-muted px-3 py-2 text-sm outline-none text-center"
                  />
                  <input
                    type="number"
                    value={priceMax}
              ...
              >
                <div
                  className={`grid h-5 w-5 place-items-center rounded ${verifiedOnly ? "bg-success" : "bg-card border border-border"}`}
                >
