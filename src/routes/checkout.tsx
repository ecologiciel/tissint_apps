...
import {
  ChevronRight, CreditCard, Check, Lock, Wallet as WalletIcon,
  Smartphone, Crown, Sparkles,
} from "lucide-react";
...
function CheckoutPage() {
  ...
  const [methodId, setMethodId] = useState(paymentMethods.find((p) => p.isDefault)?.id ?? paymentMethods[0]?.id);
  ...
  return (
    <div className="flex h-full flex-col bg-warm" dir="rtl">
      <header className="bg-navy text-warm px-5 pt-12 pb-5 rounded-b-3xl shadow-lg flex items-center justify-between">
        ...
        <span className="text-[10px] flex items-center gap-1 text-gold"><Lock className="h-3 w-3" /> آمن</span>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
              ...
              return (
                <button key={p.key} onClick={() => setPlan(p.key)}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {p.hint && <span className="text-[10px] bg-gold/20 text-gold rounded-full px-2 py-0.5">{p.hint}</span>}
                  </div>
                  <p className="mt-2"><span className="text-xl font-black text-orange">{p.priceDh}</span><span className="text-[10px] text-muted-foreground"> د.م {p.periodLabel}</span></p>
                </button>
              );
            })}
          </div>
        ...
        <section>
            ...
            {paymentMethods.map((pm) => {
              ...
              return (
                <li key={pm.id}>
                  ...
                  >
                    <span className="grid h-10 w-10 place-items-center rounded-xl bg-muted text-navy"><Icon className="h-5 w-5" /></span>
                    <div className="flex-1 text-right">
                      <p className="text-sm font-bold">{pm.label}{pm.last4 ? ` •••• ${pm.last4}` : ""}</p>
                      {pm.kind === "wallet" && (
                        <p className="text-[11px] text-muted-foreground">الرصيد: {walletBalanceDh.toLocaleString()} د.م {walletInsufficient && "— غير كافٍ"}</p>
                      )}
                    </div>
                    <span className={`h-5 w-5 rounded-full border-2 ${active ? "border-orange bg-orange" : "border-muted"} grid place-items-center`}>
          ...
          <Link to="/billing" className="block text-xs text-orange font-bold mt-2">إدارة طرق الدفع ←</Link>
        </section>
        ...
        <section>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => toast(discount > 0 ? "تم تطبيق الخصم" : "رمز غير صالح")}
              className="rounded-xl bg-navy text-warm px-4 text-sm font-bold">تطبيق</button>
          </div>
        </section>

        {/* Summary */}
        <section className="rounded-2xl bg-card border border-border p-4 space-y-1.5 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">المجموع الفرعي</span><span>{subtotal.toLocaleString()} د.م</span></div>
          {discount > 0 && (
            <div className="flex justify-between text-success"><span>خصم</span><span>-{discount} د.م</span></div>
          )}
          <div className="flex justify-between text-muted-foreground text-xs"><span>منها TVA 20٪</span><span>{(total - total / 1.2).toFixed(2)} د.م</span></div>
          <div className="border-t border-border my-2" />
          <div className="flex justify-between font-black text-base"><span>المجموع</span><span className="text-orange">{total.toLocaleString()} د.م</span></div>
        </section>
      ...
      <div className="border-t bg-card p-3">
        <button onClick={pay} disabled={!methodId || !accepted || processing}
        ...
        <p className="text-center text-[10px] text-muted-foreground mt-2"><Lock className="inline h-3 w-3 ml-1" /> تشفير SSL • الدفع عبر CMI / Visa</p>
      </div>
    </div>
  );
}
