...
function CertificatePage() {
  ...
  const item = lastScan?.scanId === scanId
    ? lastScan
    : collection.find((c) => c.scanId === scanId);

  if (!item) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center" dir="rtl">
        <p className="text-sm text-muted-foreground">لم يتم العثور على الفيحص</p>
        <Link to="/collection" className="rounded-full bg-orange px-5 py-2 text-white text-sm">العودة للمجموعة</Link>
      </div>
    );
  }

  const issuedAt = new Date().toLocaleDateString("ar-MA", { year: "numeric", month: "long", day: "numeric" });
  ...
  return (
    <div className="flex h-full flex-col bg-stone" dir="rtl">
      <header className="flex items-center justify-between px-5 pt-12 pb-3 bg-navy text-warm print:hidden">
        <Link to="/collection" className="grid h-10 w-10 place-items-center rounded-full bg-white/10">
      ...
      <main className="flex-1 overflow-y-auto p-5">
        <div className="mx-auto max-w-md rounded-3xl bg-warm text-navy border-4 border-double border-gold p-6 shadow-2xl space-y-4 relative overflow-hidden">
          <div className="absolute inset-0 pointer-events-none opacity-5"
            style={{ background: "repeating-linear-gradient(45deg, currentColor 0 1px, transparent 1px 14px)" }} />
          ...
          <div className="relative text-center border-b-2 border-gold/40 pb-4">
            ...
            <p className="text-[10px] uppercase tracking-[0.3em] text-navy/60">Certificate of Authenticity</p>
          ...
          <div className="relative flex items-center justify-between border-t-2 border-gold/40 pt-4">
            <div className="text-[10px] text-navy/70 leading-relaxed max-w-[60%]">
              يُشهد بأن العيّنة الموصوفة قد خضعت للتحليل البصري ضمن منصة تيسنت.
              هذه الشهادة استرشادية ولا تحلّ محلّ التحليل المخبري.
            </div>
        ...
        <div className="mx-auto max-w-md mt-5 grid grid-cols-2 gap-3 print:hidden">
          <button onClick={handlePrint} className="rounded-xl bg-card border-2 py-3 font-bold flex items-center justify-center gap-2 text-sm">
            <Printer className="h-4 w-4" /> طباعة
          </button>
          <button onClick={handleDownload} className="rounded-xl bg-orange text-white py-3 font-bold flex items-center justify-center gap-2 text-sm">
...
function verdictLabel(v: string) {
  return { likely: "احتمال قوي", possible: "احتمال متوسط", unlikely: "احتمال ضعيف", rejected: "مرفوض" }[v] || v;
}
