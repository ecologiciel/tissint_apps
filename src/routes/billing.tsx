      ...
      <main className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
              ...
              <>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-bold">
                      Premium {premiumPlan === "yearly" ? "سنوي" : "شهري"}
                    </p>
                  </div>
                ...
                {premiumRenewsAt && (
                  <p className="text-xs text-muted-foreground mt-2">
                    يتجدد في {new Date(premiumRenewsAt).toLocaleDateString("ar")}
                  </p>
                )}
                <button
                  onClick={() => {
        ...
        <section className="rounded-2xl bg-card border border-border overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b bg-warm/30">
            ...
            <button
              onClick={() => toast("نموذج إضافة بطاقة (محاكاة)")}
          ...
          <ul className="p-2 space-y-1">
            {paymentMethods.map((pm) => {
              ...
              return (
                <li
                  key={pm.id}
                  className="flex items-center gap-3 rounded-xl px-2 py-2.5 hover:bg-warm/40"
                >
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-muted text-navy">
                  ...
                  <div className="flex-1">
                    ...
                    {pm.isDefault && (
                      <p className="text-[10px] text-success font-bold">الافتراضي</p>
                    )}
                  </div>
                  {!pm.isDefault && (
                    <button
              ...
              <li key={iv.id} className="flex items-center gap-3 p-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {iv.status === "paid" && (
                      <span className="text-[10px] font-bold bg-success/10 text-success rounded-full px-2 py-0.5 flex items-center gap-1">
                        <BadgeCheck className="h-3 w-3" /> مدفوعة
                      </span>
                    )}
                    {iv.status === "pending" && (
                      <span className="text-[10px] font-bold bg-gold/10 text-gold rounded-full px-2 py-0.5 flex items-center gap-1">
                        <Clock className="h-3 w-3" /> قيد الانتظار
                      </span>
                    )}
                    {iv.status === "failed" && (
                      <span className="text-[10px] font-bold bg-destructive/10 text-destructive rounded-full px-2 py-0.5 flex items-center gap-1">
                        <XCircle className="h-3 w-3" /> فاشلة
                      </span>
                    )}
                  </div>
                  <p className="text-[11px] text-muted-foreground">
                    {iv.number} · {new Date(iv.createdAt).toLocaleDateString("ar")}
                  </p>
                </div>
                <p className="text-sm font-black text-orange">{iv.totalDh} د.م</p>
                <button
                  onClick={() => toast("تنزيل PDF (محاكاة)")}
        ...
        <Link
          to="/wallet"
          className="block rounded-2xl bg-card border border-border p-4 flex items-center gap-3"
        >
