...
import {
  ChevronRight, Check, X, ShieldCheck, Users, Flag, BarChart3, Store,
  TrendingUp, TrendingDown, Crown, ScanLine, AlertTriangle,
} from "lucide-react";
...
const MOCK_USERS = [
  { id: "u1", name: "Yassine A.", role: "premium", scans: 142, listings: 8, joined: "2025-11-12", flagged: false },
  { id: "u2", name: "Khalid B.", role: "free", scans: 12, listings: 1, joined: "2026-01-04", flagged: false },
  { id: "u3", name: "Soukaina M.", role: "premium", scans: 89, listings: 5, joined: "2025-09-30", flagged: false },
  { id: "u4", name: "Mehdi T.", role: "free", scans: 3, listings: 0, joined: "2026-01-22", flagged: true },
  { id: "u5", name: "Fatima Z.", role: "premium", scans: 211, listings: 12, joined: "2025-07-18", flagged: false },
];

const MOCK_REPORTS = [
  { id: "r1", target: "إعلان: نيزك Erfoud 280g", reason: "صور غير مطابقة", reporter: "Yassine A.", at: "منذ 2 ساعة", severity: "high" },
  { id: "r2", target: "مستخدم: Mehdi T.", reason: "سلوك مشبوه", reporter: "Soukaina M.", at: "منذ 5 ساعات", severity: "medium" },
  { id: "r3", target: "إعلان: Chondrite 1.2kg", reason: "سعر مبالغ فيه", reporter: "Khalid B.", at: "أمس", severity: "low" },
];
...
function AdminPage() {
  ...
  if (!unlocked) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-8 bg-stone text-warm gap-4" dir="rtl">
        <ShieldCheck className="h-12 w-12 text-gold" />
        <h1 className="text-xl font-bold">لوحة الإدارة</h1>
        <input type="password" value={pwd} onChange={(e) => setPwd(e.target.value)}
          placeholder="كلمة المرور" className="w-full rounded-xl bg-white/10 p-3 text-center outline-none" />
        <button onClick={() => {
          if (pwd === "tissint2026") { setUnlocked(true); setRole("admin"); toast.success("مرحباً Admin"); }
          else toast.error("كلمة مرور خاطئة");
        }} className="w-full rounded-full bg-orange py-3 font-bold text-white">دخول</button>
        <Link to="/dashboard" className="text-xs text-warm/50">العودة</Link>
      </div>
    );
  }
        ...
        {[
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id as Tab)}
              ...
              {t.badge ? (
                <span className="ml-1 grid h-4 min-w-4 px-1 place-items-center rounded-full bg-destructive text-[9px] text-white">{t.badge}</span>
              ) : null}
            </button>
          );
        })}
        ...
        {tab === "listings" && (
          <>
            ...
            <section>
              ...
              {pending.length === 0 ? (
                <p className="text-sm text-muted-foreground rounded-xl bg-muted p-4 text-center">لا توجد إعلانات للمراجعة</p>
              ) : (
                <div className="space-y-2">
                  {pending.map((l) => (
                    <div key={l.id} className="flex gap-3 rounded-xl bg-card border p-3">
                      <MeteoriteThumb seed={l.imageSeed} className="h-16 w-16 rounded-lg shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold truncate">{l.title}</p>
                        <p className="text-xs text-muted-foreground">{l.priceDh} درهم • {l.region}</p>
                        <div className="mt-2 flex gap-1">
                          <button onClick={() => act(l.id, "approved")} className="flex-1 rounded-lg bg-success text-white py-1.5 text-xs font-bold flex items-center justify-center gap-1"><Check className="h-3 w-3" />قبول</button>
                          <button onClick={() => act(l.id, "rejected")} className="flex-1 rounded-lg bg-destructive text-white py-1.5 text-xs font-bold flex items-center justify-center gap-1"><X className="h-3 w-3" />رفض</button>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <section>
              ...
              <div className="space-y-1.5">
                {listings.map((l) => (
                  <div key={l.id} className="flex items-center gap-2 rounded-lg bg-card border p-2 text-xs">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                      ...
                      l.status === "pending" ? "bg-warning/20 text-warning" :
                      l.status === "sold" ? "bg-primary/20 text-primary" :
        ...
        {tab === "users" && (
          <>
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              <Stat label="مبلَّغ عنهم" value={MOCK_USERS.filter(u => u.flagged).length} color="text-destructive" />
            </div>
            <div className="space-y-2">
              {MOCK_USERS.map((u) => (
                <div key={u.id} className="rounded-xl bg-card border p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-muted-foreground">منذ {u.joined} · {u.scans} فحص · {u.listings} إعلان</p>
                    </div>
                    <button onClick={() => toast.success("تم تعليق الحساب")}
                  ...
                  <div key={r.id} className="rounded-xl bg-card border p-3">
                    <div className="flex items-start gap-2">
                      <span className={`mt-1 h-2 w-2 rounded-full shrink-0 ${
                        r.severity === "high" ? "bg-destructive" :
                        r.severity === "medium" ? "bg-warning" : "bg-muted-foreground"
                      }`} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold">{r.target}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">{r.reason}</p>
                        <p className="text-[10px] text-muted-foreground mt-1">من {r.reporter} · {r.at}</p>
                      </div>
                    </div>
                    <div className="mt-2 flex gap-1">
                      <button onClick={() => resolveReport(r.id)} className="flex-1 rounded-lg bg-success text-white py-1.5 text-xs font-bold">حل</button>
                      <button onClick={() => resolveReport(r.id)} className="flex-1 rounded-lg bg-muted text-foreground py-1.5 text-xs font-bold">تجاهل</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {tab === "stats" && (
          <>
            <div className="grid grid-cols-2 gap-3">
              <BigStat label="المداخيل (شهر)" value={`${revenueDh.toLocaleString()} د.م`} delta="+18%" up icon={TrendingUp} />
              <BigStat label="الفحوصات" value={scansTotal.toLocaleString()} delta="+24%" up icon={ScanLine} />
                ...
                {[
                  ...
                  <div key={r.name}>
                    ...
                    <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-orange to-gold" style={{ width: `${r.pct}%` }} />
                    </div>
                  </div>
                ))}
              </div>
...
function BigStat({ label, value, delta, up, icon: Icon }: { label: string; value: string | number; delta: string; up: boolean; icon: any }) {
  return (
    <div className="rounded-2xl bg-card border p-4">
      <div className="flex items-center justify-between">
        <Icon className="h-4 w-4 text-orange" />
        <span className={`text-[10px] font-bold ${up ? "text-success" : "text-destructive"}`}>{delta}</span>
      </div>
...
function Health({ label, status, value }: { label: string; status: "ok" | "warn" | "down"; value: string }) {
