...
function ListingPage() {
  ...
  if (!l)
    return (
      <div className="p-6" dir="rtl">
        <p>غير موجود</p>
        <Link to="/market" className="text-orange">
          العودة
        </Link>
      </div>
    );
  ...
  const shareUrl =
    typeof window !== "undefined" ? `${window.location.origin}/market/${l.id}` : `/market/${l.id}`;
  ...
  return (
    <div className="flex h-full flex-col" dir="rtl">
      <header className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-5 pt-12 pb-3">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              toggleFavorite(l.id);
              toast.success(isFav ? "أُزيلت" : "أُضيفت للمفضلات");
            }}
        ...
        <div className="p-5 space-y-4 -mt-6 relative bg-background rounded-t-3xl">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-bold text-orange bg-orange/10 rounded-full px-2 py-0.5">
                {l.priceMode === "fixed"
                  ? "سعر ثابت"
                  : l.priceMode === "negotiable"
                    ? "قابل للتفاوض"
                    : "مزاد"}
              </span>
              {l.sellerVerified && (
                <span className="text-xs font-bold text-success bg-success/10 rounded-full px-2 py-0.5 flex items-center gap-1">
                  <BadgeCheck className="h-3 w-3" /> بائع موثوق
                </span>
              )}
            </div>
            <h1 className="text-xl font-black">{l.title}</h1>
            <p className="text-sm text-muted-foreground">{l.classification}</p>
            <p className="text-2xl font-black text-orange mt-2">
              {l.priceDh.toLocaleString()} درهم
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-xl bg-muted p-2">
              ...
              <p className="font-bold">{l.weightG} g</p>
            </div>
            <div className="rounded-xl bg-muted p-2">
              <p className="text-muted-foreground">نقاط AI</p>
              <p className="font-bold">{l.score}/100</p>
            </div>
            <div className="rounded-xl bg-muted p-2">
          ...
          <div className="rounded-xl bg-muted p-3 flex items-start gap-2 text-xs text-muted-foreground">
            ...
            <span>
              التواصل خارج التطبيق. لا تدفع قبل الفحص الفعلي. Tissint غير مسؤولة عن المعاملات.
            </span>
          </div>
        </div>
      </main>
      ...
      <ShareSheet
        ...
        title={l.title}
        text={`اكتشف ${l.title} على Tissint`}
        url={shareUrl}
      />
    </div>
  );
