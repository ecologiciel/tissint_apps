...
function ItemPage() {
  ...
  if (!item) return (
    <div className="p-6 text-center" dir="rtl">
      <p>غير موجود</p>
      <Link to="/collection" className="text-orange">العودة</Link>
    </div>
  );
  ...
  return (
    <div className="flex h-full flex-col" dir="rtl">
      <header className="bg-navy text-warm px-5 pt-12 pb-4 flex items-center justify-between">
        ...
        <button onClick={() => { removeFromCollection(item.id); toast.success("تم الحذف"); nav({ to: "/collection" }); }}
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          <dl className="grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl bg-muted p-3"><dt className="text-xs text-muted-foreground">الوزن</dt><dd className="font-bold">{item.weightG ?? "—"} g</dd></div>
            <div className="rounded-xl bg-muted p-3"><dt className="text-xs text-muted-foreground">المنطقة</dt><dd className="font-bold">{item.origin ?? "—"}</dd></div>
            <div className="rounded-xl bg-muted p-3 col-span-2"><dt className="text-xs text-muted-foreground">التاريخ</dt><dd className="font-bold">{item.createdAt}</dd></div>
          </dl>
          {item.notes && <p className="text-sm text-muted-foreground">{item.notes}</p>}
          <button onClick={() => nav({ to: "/market" })}
