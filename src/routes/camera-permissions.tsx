...
function PermissionsPage() {
  ...
  const [state, setState] = useState<Record<PermKey, PermState>>({
    camera: "idle", location: "idle", notifications: "idle",
  });

  const perms: { key: PermKey; icon: any; title: string; desc: string; required: boolean }[] = [
    { key: "camera", icon: Camera, title: "الكاميرا", desc: "لمسح الأحجار وتحليلها بصرياً", required: true },
    { key: "location", icon: MapPin, title: "الموقع", desc: "لتحديد منطقة الاكتشاف (اختياري)", required: false },
    { key: "notifications", icon: Bell, title: "الإشعارات", desc: "تنبيهات الرسائل والصفقات", required: false },
  ];
  ...
  return (
    <div className="flex h-full flex-col bg-background" dir="rtl">
      <header className="flex items-center gap-3 px-5 pt-12 pb-4">
        <Link to="/onboarding" className="grid h-10 w-10 place-items-center rounded-full bg-card border">
      ...
      <main className="flex-1 overflow-y-auto px-5 space-y-3">
        <p className="text-sm text-muted-foreground mb-2">يحتاج تيسنت لبعض الأذونات للعمل بأفضل شكل</p>
        {perms.map((p) => {
          const Icon = p.icon;
          const s = state[p.key];
          return (
            <div key={p.key} className="rounded-2xl bg-card border p-4 flex items-center gap-3">
              <div className={`grid h-12 w-12 place-items-center rounded-xl ${s === "granted" ? "bg-success/15 text-success" : "bg-primary/10 text-primary"}`}>
                {s === "granted" ? <Check className="h-6 w-6" /> : <Icon className="h-6 w-6" />}
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-sm">{p.title}</h3>
                  {p.required && <span className="text-[10px] rounded-full bg-orange/15 text-orange px-2 py-0.5">إلزامي</span>}
                </div>
      ...
      <footer className="p-5 space-y-2">
        ...
        <button onClick={() => nav({ to: "/dashboard" })} className="w-full text-xs text-muted-foreground py-2">
          تخطي الآن
        </button>
      </footer>
    </div>
