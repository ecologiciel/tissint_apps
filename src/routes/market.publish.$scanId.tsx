...
function suggestPrice(classification: string, weightG: number, rare: boolean) {
  const entry =
    PRICE_TABLE.find((p) => p.match.test(classification)) ?? PRICE_TABLE[PRICE_TABLE.length - 1];
  const raw = entry.basePerG * weightG * (rare ? 1.6 : 1);
  return {
    price: Math.max(50, Math.round(raw / 10) * 10),
    family: entry.label,
    basePerG: entry.basePerG,
  };
}
...
function PublishPage() {
  ...
  const source =
    lastScan?.scanId === scanId ? lastScan : collection.find((c) => c.scanId === scanId);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!agree) {
      toast.error("الرجاء قبول شروط البيع");
      return;
    }
    setLoading(true);
    ...
    <div className="flex h-full flex-col bg-warm" dir="rtl">
      ...
      <form onSubmit={submit} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="rounded-2xl bg-gradient-to-l from-success/15 via-gold/10 to-orange/10 border-2 border-success/40 p-4 animate-fade-in">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-bold text-success">
                موثّق تيسينت — تم التعبئة التلقائية
              </span>
            </div>
        ...
        <div className="rounded-2xl bg-card border border-border p-3 flex items-center gap-3 relative overflow-hidden">
          ...
          {isRare && (
            <span className="absolute bottom-2 left-2 rounded-full bg-gold text-white text-[9px] font-bold px-2 py-0.5">
              ⭐ نادرة
            </span>
          )}
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {photos.map((p, i) => (
              <div key={i} className="relative aspect-square">
                ...
                {i === 0 && (
                  <span className="absolute bottom-1 right-1 rounded bg-orange text-white text-[9px] px-1.5 py-0.5 font-bold">
                    الرئيسية
                  </span>
                )}
              </div>
        ...
        <Section title="معلومات العينة" icon={Tag}>
          <Field label="عنوان الإعلان *">
            <input
              ...
              placeholder="مثال: شوندريت H5 — ورزازات"
              className={inputCls}
            />
          </Field>
        ...
        <Section title="السعر" icon={Tag}>
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-3 mb-3">
            ...
            <p className="text-sm font-bold tabular-nums">
              {suggestion.price.toLocaleString()} د.م
            </p>
          ...
          {price && (
            <p className="text-[11px] text-muted-foreground -mt-2">
              ...
              <span className="font-bold text-stone">
                {(parseFloat(price) * 0.05).toFixed(0)} د.م
              </span>{" "}
              · صافي:{" "}
              <span className="font-bold text-orange">
                {(parseFloat(price) * 0.95).toFixed(0)} د.م
              </span>
            </p>
          )}
        </Section>
        ...
        <Section title="الوصف" icon={FileText}>
          <textarea
            ...
            rows={4}
            maxLength={500}
            placeholder="اذكر تفاصيل العثور، التاريخ، الحالة، الشهادات…"
        ...
        <label className="flex items-start gap-2 text-xs text-stone/70">
          ...
          />
          <span>
            أؤكد أن العينة مطابقة للوصف، وأوافق على{" "}
            <span className="text-orange font-bold">شروط البيع</span> وعمولة 5%.
          </span>
        </label>
...
function SuccessOverlay({
  ...
  return (
    <div
      className="flex h-full flex-col bg-gradient-to-b from-success/10 via-warm to-warm overflow-y-auto"
      dir="rtl"
    >
      {/* Confetti */}
        ...
        <div className="rounded-2xl bg-card border-2 border-success/30 p-4 shadow-xl animate-scale-in">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-lg font-black text-orange tabular-nums">
                {price.toLocaleString()} د.م
              </p>
            </div>
            {certified && (
            ...
            <div className="space-y-2">
              {crossSell.map((l) => (
                ...
                >
                  ...
                  <p className="text-sm font-bold text-orange tabular-nums">
                    {l.priceDh.toLocaleString()}
                  </p>
                </Link>
              ))}
            </div>
            <Link to="/market" className="mt-3 block text-center text-xs font-bold text-primary">
              استكشف السوق ←
            </Link>
          </section>
        )}
...
const inputCls =
  "w-full rounded-xl border border-stone/15 bg-white px-3 py-2.5 text-sm outline-none focus:border-orange focus:ring-2 focus:ring-orange/20";
