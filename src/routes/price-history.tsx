...
function PriceHistoryPage() {
  ...
  const avgPrice = matching.length
    ? Math.round(
        matching.reduce((s, l) => s + l.priceDh / Math.max(l.weightG, 1), 0) / matching.length,
      )
    : 0;
      ...
      <main className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div className="rounded-2xl bg-card border p-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <span
              className={`flex items-center gap-1 text-xs font-bold ${trend >= 0 ? "text-success" : "text-destructive"}`}
            >
              {trend >= 0 ? (
          ...
          <p className="text-3xl font-black text-primary">
            {avgPrice} <span className="text-sm text-muted-foreground">د.م/غ</span>
          </p>
